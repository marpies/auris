//
//  DefaultLibraryContentRepository.swift
//  Auris
//
...
struct DefaultLibraryContentRepository: LibraryContentRepository {
    private static let requestLimit = 25
    
    func load() async throws -> LibraryContent {
        let items = try await loadMusicItems()
        return LibraryContent(items: items)
    }
